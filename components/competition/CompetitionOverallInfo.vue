  ...
  <div>
    <section class="md:grid grid-cols-3 grid-rows-2 gap-12 mx-auto mt-12 container">
      ...
      >
        ...
        <IcPodium class="h-20" />
        <span class="text-4xl lg:text-5xl font-league-gothic">{{ info.weekly_prize_status }}</span>
      </div>
      <div
        v-if="lastWeekWinner"
...
export default {
  ...
  computed: {
    lastWeekWinner() {
      return this.info.last_week_winners[0]
    },

    topScorer() {
      return this.info.general_top_scorers[0]
    },
  },
}
</script>
